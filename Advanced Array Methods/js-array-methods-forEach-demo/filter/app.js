...
const longWord = words.filter(function(word) {
	return word.length >= 20;
		...
		const cWords =  words.filter(function(w){
			return w[0] === 'u' || w[0] === 'c';
		})
const vWords = words.filter(function(v){
	return v.includes('a', 'e', 'i', 'o', 'u');
});
const nWords = words.filter(function(n){
	return n.includes('a', 'e', 'i', 'o', 'u');
});
...
const everyOtherWords = myFilter(words, function(word, i) {
	return i % 2 === 0;
});
